...
The Toeplitz matrix generated by the finite window is:

```text
T_{d,n}(r,c) = B^{d,n,0}_{c-r}
```

with entries outside `0..d` set to zero. The target asks for every minor of
this finite Toeplitz matrix to be nonnegative, for every `d` and `n`.
...
degree-2 signed-Hankel identity. The exact algebra gate:
...
```text
for every d,n, the finite Toeplitz matrix generated by
