...
cone-buffer rows: 63 / 63
one-third width buffer rows: 63 / 63
defect monotone rows: 60 / 60
scaled-defect increase rows: 60 / 60
```

Minimum and maximum sampled margins:

```text
min upper defect d_k: 6.008370137537300077E-3 at lambda=-100.0, k=21
min lower-wall slack: 3.313293291898931350E-2 at lambda=-25.0, k=21
min one-third buffer margin: 4.569139109160357364E-2 at lambda=-25.0, k=21
min defect monotone gap: 7.539431011168075104E-5 at lambda=-100.0, k=20
max scaled defect: 2.876419422417297597E-1 at lambda=-25.0, k=21
max defect ratio d_{k+1}/d_k: 9.876072930238421307E-1 at lambda=-100.0, k=20
```
...
The current finite-collar contract has active depth `K=19` with collars `x_20` and `x_21`. The next purely finite upgrade to `K=20` needs
`x_22`, hence `A_23`, unless an analytic tail theorem supplies the
missing collar.
...
The certified prefix satisfies the defect-form cone barriers 0 <= d_k <= 2/(2k+1), d_{k+1} <= d_k, and the stronger finite one-third width buffer d_k <= 2/(3*(2k+1)) through x_21. However, the scaled defect is increasing on the checked prefix, so any analytic proof route that assumes scaled-defect monotone decrease is already rejected. The next finite collar needs x_22, hence A_23, unless an analytic tail theorem supplies it.
