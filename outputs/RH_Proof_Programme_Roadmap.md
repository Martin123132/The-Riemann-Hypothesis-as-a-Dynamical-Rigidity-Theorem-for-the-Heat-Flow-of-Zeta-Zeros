...
- The Grussler-Damm sign-consistency route has been sharpened: shifted-principal signed-Hankel minors are not enough, so a reshaped-Hankel point audit checks all k-column minors of the k-row matrix `(A_{i+j})` for `k = 2..5`, `N = 18`, across the five-lambda grid. The exact-rationalized cache audit is clean, but it is not an interval certificate or all-order theorem.
