...
- The Grussler-Damm sign-consistency route has been sharpened: shifted-principal signed-Hankel minors are not enough, so a reshaped-Hankel audit checks all k-column minors of the k-row matrix `(A_{i+j})` across the five-lambda grid. The exact-rationalized cache audit is now accompanied by an Arb/enclosure-backed finite certificate through `k = 2..7`, `N = 20`, validating `689,795/689,795` finite minors. This is still not an all-order sign-consistency theorem.
