...
```text
python work/rh_compute/scripts/check_jensen_window_pf_negative_lambda_finite_collar_contract.py --target work/rh_compute/results/jensen_window_pf_negative_lambda_finite_collar_k100_contract.json --note outputs/jensen_window_pf_negative_lambda_finite_collar_k100_contract.md
```
...
available coefficient range: A_0..A_100
available ratio range: x_1..x_99
lower/upper range: x_1..x_99
monotone-gap range: 1 <= k <= 98

certified active depth: K=97
first collar: x_98
second collar: x_99
```
...
```text
active lower rows: 291 / 291
active upper rows: 291 / 291
active monotone rows: 291 / 291
collar lower/upper rows: 6 / 6
collar monotone rows: 3 / 3

minimum active lower margin:
  5.458628018046818002E-3 at lambda=-25.0, k=97

minimum active upper margin:
  3.380229139004779895E-3 at lambda=-100.0, k=97

minimum active monotone gap:
  1.808119739573407679E-5 at lambda=-100.0, k=97

minimum collar lower/upper margin:
  3.362147941609045818E-3 at lambda=-100.0, k=98

minimum collar monotone gap:
  1.787679020491334833E-5 at lambda=-100.0, k=98
```

## Meaning

The negative-lambda prefix is now precisely collar-accounted: it can seed the
active finite depth `K=97` with collars
`x_98` and `x_99`.
It does not seed `K=98` because the
monotone-wall collar would require `x_100`,
hence additional coefficient enclosures through
`A_101`, or an analytic tail theorem.
