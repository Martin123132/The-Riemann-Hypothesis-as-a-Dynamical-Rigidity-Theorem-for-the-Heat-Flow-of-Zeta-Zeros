...
work/rh_compute/results/acb_enclosures_negative_lambda_cone_entry_lam_m25_m50_m100_k0_k50_summary.json
```
...
```text
python work/rh_compute/scripts/check_jensen_window_pf_negative_lambda_cone_entry_prefix_scout.py --target work/rh_compute/results/jensen_window_pf_negative_lambda_cone_entry_prefix_k50_scout.json --note outputs/jensen_window_pf_negative_lambda_cone_entry_prefix_k50_scout.md
```

Current result:

```text
validated Jensen-window PF negative-lambda cone-entry prefix scout: 153 coefficient rows, 147 lower-wall rows, 147 upper-wall rows, 144 monotone-gap rows, 0 issues
```
...
lower and upper walls: 1 <= k <= 49
monotone wall: 1 <= k <= 48
```

The coefficient enclosures were generated by:

```text
python work/rh_compute/scripts/acb_coefficient_enclosures.py --lambdas=-25,-50,-100 --k-min 0 --k-max 50 --n-sum 50 --cutoff 6 --dps 160 --digits 80 --abs-tol 1e-110 --constant-terms 30 --output-dir work/rh_compute/results --run-id acb_enclosures_negative_lambda_cone_entry_lam_m25_m50_m100_k0_k50 --overwrite
```

## Certified Margins

```text
lower wall rows: 147 / 147
upper wall rows: 147 / 147
monotone gap rows: 144 / 144

minimum lower-wall margin:
  1.231199502558002468E-2 at lambda=-25.0, k=49

minimum upper-wall margin:
  4.598108723184657489E-3 at lambda=-100.0, k=49
