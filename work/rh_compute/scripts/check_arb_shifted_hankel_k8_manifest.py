#!/usr/bin/env python3
"""Validate shifted Arb reshaped-Hankel order-8 certificate artifacts."""
...
def configure_order_8_spec() -> None:
    shifted_checker.DEFAULT_SUMMARY = DEFAULT_SUMMARY
    shifted_checker.DEFAULT_JSONL = DEFAULT_JSONL
    shifted_checker.EXPECTED_ORDERS = (8,)
    shifted_checker.EXPECTED_N_COLS = 14
...
def main() -> int:
    ...
    if args.json:
        ...
        for issue in issues:
            print(f"SHIFTED-ARB-HANKEL-K8 {issue.row_id} [{issue.issue}] {issue.detail}")
        print(
            "validated "
            f"{summary.get('rows')} shifted Arb reshaped-Hankel order-8 finite certificates "
