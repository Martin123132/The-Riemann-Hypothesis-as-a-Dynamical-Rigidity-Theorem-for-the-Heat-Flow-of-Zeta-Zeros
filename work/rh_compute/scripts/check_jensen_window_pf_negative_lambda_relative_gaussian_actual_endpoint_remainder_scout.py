#!/usr/bin/env python3
"""Validate the actual endpoint relative-Gaussian remainder scout."""
...
from jensen_window_pf_negative_lambda_relative_gaussian_actual_endpoint_remainder_scout import (  # noqa: E402
    DEFAULT_INDICES,
    DEFAULT_NOTE,
    DEFAULT_OUT_JSON,
...
REQUIRED_ROW_IDS = {
    "nlrgaeprs_01_actual_gamma_expectation_coordinate",
    "nlrgaeprs_02_endpoint_quadrature_samples",
    "nlrgaeprs_03_first_omitted_comparison",
    "nlrgaeprs_04_budget_comparison",
    "nlrgaeprs_05_uniform_promotion_rejected",
    "nlrgaeprs_06_acceptance_gate",
}
...
class EndpointScoutIssue:
    section: str
    issue: str
    detail: str


def issue(section: str, name: str, detail: str) -> EndpointScoutIssue:
    return EndpointScoutIssue(section=section, issue=name, detail=detail)
...
def validate_ref(section: str, ref: object) -> list[EndpointScoutIssue]:
...
def validate_top_level(artifact: dict) -> list[EndpointScoutIssue]:
    issues: list[EndpointScoutIssue] = []
    if artifact.get("kind") != "jensen_window_pf_negative_lambda_relative_gaussian_actual_endpoint_remainder_scout":
        issues.append(issue("<artifact>", "bad-kind", repr(artifact.get("kind"))))
    if artifact.get("status") != "floating endpoint theorem-search diagnostic":
...
def validate_recomputed(artifact: dict, residual_path: Path) -> list[EndpointScoutIssue]:
    try:
...
def validate_diagnostics(artifact: dict) -> list[EndpointScoutIssue]:
    ...
    expected_params = {
        ...
        "quadrature_orders": [96, 128, 192, 256, 320],
        "selected_quadrature_order": 320,
        "phi_n_terms": 23,
        "ratio_cutoff_n": 80,
        ...
        "normalizing_phi0_float": "4.466969004671234589E-01",
    }
    issues: list[EndpointScoutIssue] = []
    if params != expected_params:
        issues.append(issue("parameters", "bad-parameters", repr(params)))
    endpoint_rows = diagnostics.get("endpoint_rows", [])
    if [row.get("index") for row in endpoint_rows] != [21, 22, 23, 24]:
        issues.append(issue("endpoint_rows", "bad-index-order", repr([row.get("index") for row in endpoint_rows])))
    if diagnostics.get("endpoint_row_count") != 4:
        issues.append(issue("diagnostics", "bad-endpoint-row-count", repr(diagnostics.get("endpoint_row_count"))))
    if diagnostics.get("quadrature_order_count") != 5:
        issues.append(issue("diagnostics", "bad-quadrature-order-count", repr(diagnostics.get("quadrature_order_count"))))
    if diagnostics.get("all_selected_residuals_below_first_omitted_term") is not True:
        issues.append(issue("diagnostics", "selected-residuals-not-below-first", "expected true"))
    scalar_thresholds = {
        "max_value_residual_to_first_omitted_ratio_over_orders": 0.81,
        "max_derivative_residual_to_first_omitted_ratio_over_orders": 0.80,
        "max_value_residual_spread_scaled": 1.0e-6,
        "max_derivative_residual_spread_scaled": 1.0e-9,
        "max_value_budget_fraction_at_selected_order": 1.0e-3,
        "max_derivative_budget_fraction_at_selected_order": 1.0e-3,
    }
    for key, upper in scalar_thresholds.items():
        try:
    ...
    for row in endpoint_rows:
        ...
        for key in (
            "value_residual_to_first_omitted_ratio_at_selected_order",
            "derivative_residual_to_first_omitted_ratio_at_selected_order",
            ...
            if not 0.0 < value < 1.0:
                issues.append(issue(label, f"ratio-outside-open-unit-interval-{key}", str(value)))
        if "floating endpoint" not in str(row.get("proof_boundary", "")).lower():
            issues.append(issue(label, "weak-row-boundary", repr(row.get("proof_boundary"))))
    scope = str(diagnostics.get("floating_scope_note", "")).lower()
    for required in ("endpoint quadrature", "n=96..320", "t=1156", "double-precision cancellation", "not promoted"):
        if required not in scope:
            issues.append(issue("diagnostics", "weak-floating-scope-note", required))
    return issues


def validate_rows(artifact: dict) -> tuple[list[EndpointScoutIssue], int, int, int, int, int]:
...
def validate_summary(
    ...
    expected_exact = {
        ...
        "endpoint_rows": 4,
        "quadrature_orders": [96, 128, 192, 256, 320],
        "quadrature_order_count": 5,
        "selected_quadrature_order": 320,
        "all_selected_residuals_below_first_omitted_term": True,
    ...
    threshold_checks = {
        "max_selected_value_residual_to_first_omitted_ratio": 0.81,
        "max_selected_derivative_residual_to_first_omitted_ratio": 0.80,
        "max_value_budget_fraction_at_selected_order": 1.0e-3,
        "max_derivative_budget_fraction_at_selected_order": 1.0e-3,
    }
    for key, upper in threshold_checks.items():
        try:
    ...
    for required in (
        ...
        "actual relative-gaussian",
        "below one first omitted",
        "below 0.1%",
        "floating endpoint scout",
        "rather than a uniform analytic",
    ):
        if required not in finding:
            issues.append(issue("summary", "missing-main-finding-text", required))
    invariants = " ".join(str(item) for item in artifact.get("invariants", [])).lower()
    for required in ("ready_to_apply", "not promoted", "not interval", "numerical evidence", "lambda <= 0"):
        if required not in invariants:
            issues.append(issue("invariants", "missing-invariant-text", required))
    return issues


def validate_note(path: Path) -> list[EndpointScoutIssue]:
    if not path.exists():
...
def validate(target_path: Path, note_path: Path, residual_path: Path) -> tuple[list[EndpointScoutIssue], dict]:
...
def main() -> int:
    ...
    if args.json:
        ...
        for item in issues:
            print(f"JWPF-NEG-LAMBDA-REL-GAUSS-ACTUAL-ENDPOINT {item.section} [{item.issue}] {item.detail}")
        print(
            ...
            f"{summary.get('endpoint_rows')} endpoint rows, "
            f"{summary.get('quadrature_order_count')} quadrature orders, "
