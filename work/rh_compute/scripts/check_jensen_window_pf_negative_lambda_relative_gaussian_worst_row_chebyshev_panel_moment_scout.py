#!/usr/bin/env python3
"""Validate the worst-row Chebyshev panel-moment scout."""
...
from jensen_window_pf_negative_lambda_relative_gaussian_worst_row_chebyshev_panel_moment_scout import (  # noqa: E402
...
REQUIRED_ROW_IDS = {
    "nlrgwrcpms_01_raw_panel_rejection_import",
    "nlrgwrcpms_02_panel_moment_formula",
    "nlrgwrcpms_03_degree_ladder",
    "nlrgwrcpms_04_cauchy_delta_vs_caps",
    "nlrgwrcpms_05_reference_panel_contributions",
    "nlrgwrcpms_06_promotion_rejected",
    "nlrgwrcpms_07_acceptance_gate",
}

ALLOWED_ROLES = {
    "scope_reduction",
    "floating_panel_moment_diagnostic",
    "rejected_route",
    "acceptance_gate",
}
...
REQUIRED_NOTE_STRINGS = (
    ...
    "Arb Chebyshev coefficient balls",
...
class ChebyshevScoutIssue:
    section: str
    issue: str
    detail: str


def issue(section: str, name: str, detail: str) -> ChebyshevScoutIssue:
    return ChebyshevScoutIssue(section=section, issue=name, detail=detail)
...
def validate_ref(section: str, ref: object) -> list[ChebyshevScoutIssue]:
...
def validate_top_level(artifact: dict) -> list[ChebyshevScoutIssue]:
    issues: list[ChebyshevScoutIssue] = []
    if (
        artifact.get("kind")
    ...
    if artifact.get("status") != "worst-row floating Chebyshev panel-moment scout":
        issues.append(issue("<artifact>", "bad-status", repr(artifact.get("status"))))
    for key in (
        "source_compact_interval_integration_scout",
        "source_compact_interval_integration_scout_json",
    ...
    for required in (
        "floating chebyshev panel-moment scout only",
        "does not provide arb coefficient balls",
        "does not prove interpolation",
        "compact interval-integration certificate",
...
def validate_recomputed(artifact: dict) -> list[ChebyshevScoutIssue]:
    try:
        recomputed = build_artifact(DEFAULT_COMPACT_SCOUT_JSON, DEFAULT_QUADRATURE_ROUTE_JSON)
...
def validate_rows(artifact: dict) -> tuple[list[ChebyshevScoutIssue], int, int]:
    ...
    promotion = rows_by_id.get("nlrgwrcpms_06_promotion_rejected", {})
    if "arb coefficient balls" not in str(promotion.get("gap", "")).lower():
        issues.append(issue("nlrgwrcpms_06_promotion_rejected", "weak-rejection-gap", repr(promotion.get("gap"))))
    gate = rows_by_id.get("nlrgwrcpms_07_acceptance_gate", {}).get("diagnostics", {})
    if "remainder bound" not in str(gate.get("recommended_upgrade", "")).lower():
        issues.append(issue("nlrgwrcpms_07_acceptance_gate", "weak-upgrade", repr(gate)))
    return issues, len(rows), diagnostic_rows


def validate_summary(artifact: dict, row_count: int, diagnostic_rows: int) -> list[ChebyshevScoutIssue]:
    ...
    expected = {
        ...
        "alpha": "20.5",
        "mpmath_dps": 100,
        "flint_precision_bits_for_coefficients": 384,
        "polynomial_M": 20,
        "ratio_cutoff_n": 80,
        "phi_term_count": 30,
        "panel_count": 6,
    ...
    for row in artifact.get("matrix_rows", []):
        if isinstance(row, dict) and row.get("id") == "nlrgwrcpms_04_cauchy_delta_vs_caps":
            cauchy_rows = row.get("diagnostics", {}).get("cauchy_rows")
    if not isinstance(cauchy_rows, list) or len(cauchy_rows) != 4:
    ...
    if by_pair.get("12->16", {}).get("both_deltas_below_caps") is not False:
        issues.append(issue("summary", "bad-12-16-cap-status", repr(by_pair.get("12->16"))))
    ...
    if dec(by_pair.get("16->20", {}).get("value_delta_to_cap_ratio", "1")) >= Decimal("0.02"):
        issues.append(issue("summary", "value-16-20-ratio-too-large", repr(by_pair.get("16->20"))))
    if dec(by_pair.get("16->20", {}).get("derivative_delta_to_cap_ratio", "1")) >= Decimal("0.02"):
        issues.append(issue("summary", "derivative-16-20-ratio-too-large", repr(by_pair.get("16->20"))))
    if dec(by_pair.get("20->24", {}).get("value_delta_to_cap_ratio", "1")) >= Decimal("1e-8"):
        issues.append(issue("summary", "value-20-24-ratio-too-large", repr(by_pair.get("20->24"))))
    if dec(by_pair.get("20->24", {}).get("derivative_delta_to_cap_ratio", "1")) >= Decimal("1e-8"):
        issues.append(issue("summary", "derivative-20-24-ratio-too-large", repr(by_pair.get("20->24"))))
    if abs(dec(summary.get("reference_value_integral_estimate", "0"))) <= Decimal("1e-35"):
        issues.append(issue("summary", "reference-value-too-small", repr(summary.get("reference_value_integral_estimate"))))
    if abs(dec(summary.get("reference_derivative_integral_estimate", "0"))) <= Decimal("1e-33"):
        issues.append(issue("summary", "reference-derivative-too-small", repr(summary.get("reference_derivative_integral_estimate"))))
    finding = str(summary.get("main_finding", "")).lower()
    for required in (
        "floating chebyshev panel-moment route",
        "numerically plausible",
        "0<=y<=200",
        "16->20",
        "arb coefficient balls",
        "panel remainder bounds",
        "calibration only",
    ):
        if required not in finding:
            issues.append(issue("summary", "weak-main-finding", required))
    upgrade = str(summary.get("recommended_upgrade", "")).lower()
    for required in ("floating", "arb coefficient balls", "remainder bound", "panel"):
    ...
    for required in ("ready_to_apply", "floating calibration", "small consecutive-degree deltas", "compact interval", "lambda <= 0"):
        if required not in invariants:
            issues.append(issue("invariants", "missing-invariant-text", required))
    return issues


def validate_note(path: Path) -> list[ChebyshevScoutIssue]:
    if not path.exists():
    ...
    for forbidden in (
        ...
        "rh is proved",
        "lambda <= 0 is proved",
        "floating chebyshev panel-moment ladder proves",
    ):
...
def main() -> int:
    ...
    if args.json:
        ...
        for item in issues:
            print(f"CHEBYSHEV-SCOUT {item.section} [{item.issue}] {item.detail}")
