#!/usr/bin/env python3
"""Validate the order-ten high-cumulant coarse corridor."""
...
def validate_artifact(path: Path) -> tuple[dict, list[CorridorIssue]]:
    ...
    if artifact.get("kind") != "jensen_window_pf_compound_order10_high_cumulant_coarse_corridor":
...
def validate_note(path: Path) -> list[CorridorIssue]:
    ...
    required = (
        ...
        "18*17=306",
        "finite residual < 323/6875",
        "ray residual < 18683/550000/u",
        "r=17,18, u>=2",
...
def main() -> int:
    ...
    if issues:
        for finding in issues:
            print(
                f"ORDER10-HIGH-CUMULANT {finding.section} "
                f"[{finding.code}] {finding.detail}"
            )
        print(f"order-ten high-cumulant corridor: {len(issues)} issues")
        return 1
    summary = artifact["summary"]
    print(
        "validated order-ten high-cumulant corridor: "
