#!/usr/bin/env python3
"""Propagate signed contiguous and arbitrary-column order seven to lambda zero."""
...
def validate_sources() -> dict:
    ...
    if entry.get("exact", {}).get("all_shift_entry") != endpoint_entry:
        raise RuntimeError("order-seven endpoint entry source changed")
    if entry.get("summary", {}).get("all_shift_m100_entry_theorems") != 1:
        raise RuntimeError("order-seven endpoint entry source is not closed")
    ...
    if flow.get("summary", {}).get("cooperative_flow_recursions") != 1:
        raise RuntimeError("order-seven cooperative flow source is not closed")
    if flow.get("summary", {}).get("conditional_forward_theorems") != 1:
        raise RuntimeError("order-seven conditional forward source is not closed")
    if flow.get("summary", {}).get("conditional_arbitrary_column_theorems") != 1:
        raise RuntimeError("order-seven conditional column source is not closed")
    if lower.get("summary", {}).get("contiguous_order_six_theorems") != 1:
        raise RuntimeError("lower order-six interval theorem changed")
    if lower.get("summary", {}).get("arbitrary_column_order_six_theorems") != 1:
    ...
    expected_flow = {
        ...
        "conditional_arbitrary_columns": (
            "completed signed contiguous layers through order seven imply "
            "every arbitrary-column signed layer through order seven"
        ),
    }
...
def conclusions() -> dict:
    return {
        ...
        "arbitrary_order_seven": (
            "epsilon_7*R_(7,n)(j_1,j_2,j_3,j_4,j_5,j_6,j_7;lambda)>0 for "
            "every n>=0, 0<=j_1<j_2<j_3<j_4<j_5<j_6<j_7, and "
            "-100<=lambda<=0"
        ),
        "fixed_order_seven": (
            "every consecutive-row arbitrary-column reshaped-Hankel minor "
            "through order seven has its required sign on -100<=lambda<=0"
        ),
        "next_handoff": (
            "extend the zeta-specific endpoint and cooperative-flow programme "
            "to order eight, while isolating a uniform-in-order mechanism"
        ),
    }
...
def build_artifact() -> dict:
    ...
    rows = [
        ForwardRow(
            "co7uhfic_01_endpoint",
            "theorem_input",
            "ready_to_apply",
            "Every signed order-seven endpoint coordinate is strictly positive at lambda=-100.",
            sources["endpoint_entry"],
            "Completed zeta-specific endpoint theorem.",
        ),
        ForwardRow(
            "co7uhfic_02_uniform_tail",
            "theorem_input",
            "ready_to_apply",
            "Signed order-seven positivity is uniform at sufficiently large shift throughout the compact heat interval.",
            sources["uniform_tail"],
            "Non-effective but compact-uniform eventual theorem.",
        ),
        ForwardRow(
            "co7uhfic_03_cooperative_flow",
            "exact_identity",
            "ready_to_apply",
            "The signed order-seven layer obeys a nearest-neighbor cooperative system with positive forward coefficient.",
            sources["cooperative_flow"],
            "Exact Plucker and heat-flow algebra inside the completed order-six cone.",
        ),
        ForwardRow(
            "co7uhfic_04_finite_confinement",
        ...
        ForwardRow(
            "co7uhfic_05_contiguous_theorem",
            "exact_forward_theorem",
            "ready_to_apply",
            "The endpoint, uniform tail, and cooperative system prove signed contiguous order seven throughout the interval.",
            theorem["contiguous_order_seven"],
            "All shifts and all finite heat parameters from -100 through zero.",
        ),
        ForwardRow(
            "co7uhfic_06_lower_layers",
            "theorem_input",
            "ready_to_apply",
            "The completed lower contiguous layers supply every initial-minor sign through order six.",
            sources["lower_layers"],
            "Previously proved lower-layer interval theorems.",
        ),
        ForwardRow(
            "co7uhfic_07_arbitrary_columns",
            "published_theorem_composition",
            "ready_to_apply",
            "The fixed-order initial-minor criterion transfers signed contiguous order seven to all arbitrary column sets.",
            theorem["arbitrary_order_seven"],
            "Finite-dimensional transfer applied pointwise in lambda.",
        ),
        ForwardRow(
            "co7uhfic_08_all_order_handoff",
            "open_theorem_target",
            "not_ready_to_apply",
            "Fixed order seven does not supply the all-order theorem required by the Jensen bridge.",
            theorem["next_handoff"],
            "Open order-eight and uniform-in-order problem; not PF-infinity, RH, or Lambda<=0.",
        ),
    ]
    return {
        "kind": "jensen_window_pf_compound_order7_uniform_heat_forward_invariance_certificate",
        "date": "2026-07-13",
        "status": (
        ...
        "proof_boundary": (
            ...
            "order seven only. It does not prove order eight or higher, "
        ...
        "generator": "work/rh_compute/scripts/jensen_window_pf_compound_order7_uniform_heat_forward_invariance_certificate.py",
        "checker": "work/rh_compute/scripts/check_jensen_window_pf_compound_order7_uniform_heat_forward_invariance_certificate.py",
...
def write_note(path: Path, artifact: dict) -> None:
    ...
    lines = [
        "# Jensen-Window PF Compound Order-Seven Uniform-Heat Forward Invariance Certificate",
        "",
        "Date: 2026-07-13",
        "",
        "Status: signed contiguous and arbitrary-column order-seven theorem on",
        ...
        "python work/rh_compute/scripts/jensen_window_pf_compound_order7_uniform_heat_forward_invariance_certificate.py",
        "python work/rh_compute/scripts/check_jensen_window_pf_compound_order7_uniform_heat_forward_invariance_certificate.py",
        "```",
        "",
        ...
        "## Arbitrary Columns",
        "",
        "The signed contiguous layers through order seven satisfy the",
        ...
        "```",
        "",
        "Thus contiguous and arbitrary-column order seven are both closed on",
...
def main() -> int:
    ...
    print(
        "wrote order-seven uniform-heat forward invariance: "
        f"{summary['rows']} rows, "
