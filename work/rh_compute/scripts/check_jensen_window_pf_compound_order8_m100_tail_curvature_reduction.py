#!/usr/bin/env python3
"""Validate the order-eight endpoint-tail curvature reduction."""
...
from jensen_window_pf_compound_order8_m100_tail_curvature_reduction import (  # noqa: E402
    CURVATURE_CONSTANT,
    DEFAULT_NOTE,
    DEFAULT_OUT,
    TAIL_FIRST_K,
...
def validate_artifact(path: Path) -> tuple[dict, list[TailIssue]]:
    ...
    if artifact.get("kind") != "jensen_window_pf_compound_order8_m100_tail_curvature_reduction":
        issues.append(issue("artifact", "bad-kind", artifact.get("kind")))
    if "one open fifth-nested curvature ceiling" not in str(artifact.get("status", "")):
    ...
    required_exact = {
        ...
        "order8_coordinate": "s(t)=2*r(t)-p(t)+log(1-exp(-U(t)))",
        ...
        "log_buffer": "-7*log(x_k)>=7*d_k>=1757/(250*(2*k+1)), k>=320",
        "rational_comparison": "4300/k^2<1757/(250*(2*k+1)), k>=1250",
    }
    for key, expected in required_exact.items():
        if exact.get(key) != expected:
            issues.append(issue("exact", f"bad-{key}", exact.get(key)))
    if exact.get("shifted_coefficients") != ["1757", "2242500", "56737500"]:
    ...
    shifted = sp.sympify(
        exact.get("shifted_polynomial_k_1250_plus_m", "0"), locals={"m": m}
    )
    expected_shifted = sp.expand(
        sp.Integer(1757) * (TAIL_FIRST_K + m) ** 2
        - sp.Integer(250) * CURVATURE_CONSTANT * (2 * (TAIL_FIRST_K + m) + 1)
    )
...
def validate_note(path: Path) -> list[TailIssue]:
    ...
    required = (
        ...
        "4300/k^2<1757/(250*(2*k+1))",
        "1757*m**2 + 2242500*m + 56737500>0",
        "Prove W_k<=4300/k^2 for every k>=1250",
        "common t+-6 cover",
        "derivatives through order fourteen",
...
def main() -> int:
    ...
    if issues:
        for finding in issues:
            print(
                f"ORDER8-TAIL-CURVATURE {finding.section} "
                f"[{finding.code}] {finding.detail}"
            )
        print(f"order-eight tail curvature reduction: {len(issues)} issues")
        return 1
    summary = artifact["summary"]
    print(
        "validated order-eight tail curvature reduction: "
