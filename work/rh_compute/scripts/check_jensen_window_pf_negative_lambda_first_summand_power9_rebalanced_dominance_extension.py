#!/usr/bin/env python3
"""Validate the inverse-ninth-power first-summand dominance extension."""
...
from jensen_window_pf_negative_lambda_first_summand_power9_rebalanced_dominance_extension import (  # noqa: E402
    DEFAULT_NOTE,
    DEFAULT_OUT,
    build_artifact,
)
...
def validate_artifact(path: Path) -> tuple[dict, list[DominanceIssue]]:
    ...
    if artifact.get("kind") != "jensen_window_pf_negative_lambda_first_summand_power9_rebalanced_dominance_extension":
        issues.append(issue("artifact", "bad-kind", artifact.get("kind")))
    if "inverse-ninth-power" not in str(artifact.get("status", "")):
    ...
    required = {
        "high_region_bound": "epsilon(a(k))<=17*exp(-3*pi*k^(2/5))<k^(-9), k>=300",
        "low_region_bound": "epsilon(0)*P_k(u<a(k))<k^(-9), k>=300",
        "full_tail_relative_bound": (
            "0<=delta_k=(M_k-M_k^(1))/M_k^(1)<2/k^9 "
            "for every integer k>=300"
        ),
        "moment_log_error": "0<=log(1+delta_k)<2/k^9, k>=300",
        "adjacent_B_error": (
...
def validate_note(path: Path) -> list[DominanceIssue]:
    ...
    required = (
        "Status: rigorous inverse-ninth-power first-summand dominance",
        "This is not a proof of order eight",
        "a(k)=log(k)/10",
        "All fourteen endpoint and derivative gates remain strictly positive",
        "<k^(-9), k>=300",
        "<2/k^9 for every integer k>=300",
...
def main() -> int:
    ...
    if issues:
        for finding in issues:
            print(
                f"POWER9-DOMINANCE {finding.section} "
                f"[{finding.code}] {finding.detail}"
            )
        print(f"power-nine first-summand dominance: {len(issues)} issues")
        return 1
    summary = artifact["summary"]
    print(
        "validated power-nine rebalanced first-summand dominance: "
