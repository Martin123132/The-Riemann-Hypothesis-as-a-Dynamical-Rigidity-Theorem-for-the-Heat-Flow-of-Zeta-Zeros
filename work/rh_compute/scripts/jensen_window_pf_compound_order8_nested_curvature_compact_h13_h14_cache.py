#!/usr/bin/env python3
"""Build the aligned H13/H14 compact-mode cache for order eight."""
...
DEFAULT_CACHE = (
    ...
    "jensen_window_pf_compound_order8_nested_curvature_compact_h13_h14_tiles.jsonl"
)
DEFAULT_MANIFEST = (
    REPO_ROOT
    / "work/rh_compute/results/"
    "jensen_window_pf_compound_order8_nested_curvature_compact_h13_h14_cache.json"
)
...
def tile_task(task: tuple[int, Fraction, Fraction]) -> dict:
    ...
    return {
        "kind": "order8_compact_h13_h14_tile",
        "index": index,
        "mode_left": str(left),
        "mode_right": str(right),
        "passed": True,
...
def load_cache(path: Path, tasks: list[tuple[int, Fraction, Fraction]]) -> list[dict]:
    ...
    if len(records) > len(tasks):
        raise RuntimeError("order-eight H13/H14 cache has too many rows")
    for record, (index, left, right) in zip(records, tasks):
        if (
            ...
            or set(record.get("h_derivatives", {})) != {"13", "14"}
        ):
            raise RuntimeError(f"order-eight H13/H14 mismatch at tile {index}")
    return records
        ...
        with path.open("a", encoding="utf-8") as handle:
            for completed, record in enumerate(iterator, start=1):
                if record.get("passed") is not True:
                    raise RuntimeError(
                        f"H13/H14 tile {record.get('index')} failed: "
                ...
                if completed % 1000 == 0:
                    handle.flush()
                    print(
                        f"order-eight compact H13/H14 tiles: "
...
def write_manifest(path: Path, cache: Path, records: list[dict]) -> dict:
    manifest = {
        "kind": "jensen_window_pf_compound_order8_nested_curvature_compact_h13_h14_cache",
        "date": "2026-07-13",
        "status": "rigorous aligned H13/H14 compact-mode interval cache; computational input only",
        "proof_boundary": (
            "This cache encloses first-summand H derivatives 13 and 14 on the "
        ...
        "generator": (
            ...
            "jensen_window_pf_compound_order8_nested_curvature_compact_h13_h14_cache.py"
        ),
    }
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
...
def main() -> int:
    ...
    print(f"order-eight compact H13/H14 cache rows: {len(records)}/{len(tasks)}")
    if args.cache_only:
        return 0
    if len(records) != len(tasks):
        raise RuntimeError("complete H13/H14 cache before writing its manifest")
    manifest = write_manifest(args.manifest, args.cache, records)
    print(
        "wrote order-eight H13/H14 cache manifest: "
