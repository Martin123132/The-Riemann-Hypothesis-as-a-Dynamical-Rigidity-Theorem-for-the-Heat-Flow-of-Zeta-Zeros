#!/usr/bin/env python3
"""Certify order-eight first-summand curvature on the ray 2<=u<=20."""
...
from jensen_window_pf_compound_order8_high_cumulant_coarse_corridor import (  # noqa: E402
    ...
    FINITE_EXACT_CAP as ULTRA_CUMULANT_CAP,
)
...
DEFAULT_EXTENSION_CACHE = (
    ...
    "jensen_window_pf_compound_order8_nested_curvature_u2_h13_h14_extension_tiles.jsonl"
)
DEFAULT_RAY_CACHE = (
    REPO_ROOT
    / "work/rh_compute/results/"
    "jensen_window_pf_compound_order8_nested_curvature_u2_u20_blocks.jsonl"
)
DEFAULT_OUT = (
    REPO_ROOT
    / "work/rh_compute/results/"
...
TOP_CUMULANT_CAP = order7_ray.HIGH_CUMULANT_CAP
...
def extension_task(task: tuple[int, Fraction, Fraction]) -> dict:
    ...
    return {
        "kind": "order8_u2_extension_h13_h14_tile",
        "index": index,
        "mode_left": str(left),
        "mode_right": str(right),
        "passed": True,
...
def choose_pad(left: Fraction, right: Fraction) -> tuple[Fraction, flint.arb]:
    ...
    while True:
        ...
        if not bool(arb_rational(pad) * curvature > COLLAR_T):
            raise RuntimeError("adaptive mode pad does not cover the t+-6 collar")
...
def corridor_h_derivatives(
    ...
    for order, cap in (
        ...
        (11, TOP_CUMULANT_CAP),
        (12, TOP_CUMULANT_CAP),
        (13, ULTRA_CUMULANT_CAP),
        (14, ULTRA_CUMULANT_CAP),
    ):
        magnitude = (
            arb_rational(cap)
...
def evaluate_dimensionless(
    ...
    *,
    ...
    try:
        ell = stable_log_series(series_scale(b, z), 12)
        J = [
            2 * b[order]
        ...
        h = series_add(
            ...
            stable_log_series(series_scale(J, z), 10),
        )
        R = [
            3 * b[order]
        ...
        q_layer = series_add(
            ...
            stable_log_series(series_scale(R, z), 8),
        )
        S = [
            4 * b[order]
        ...
        p = series_add(
            ...
            stable_log_series(series_scale(S, z), 6),
        )
        T = [
            5 * b[order]
        ...
        r_layer = series_add(
            ...
            stable_log_series(series_scale(T, z), 4),
        )
        U = [
            6 * b[order]
        ...
        s = series_add(
            ...
            stable_log_series(series_scale(U, z), 2),
        )
    ...
    coordinates = {"J": J[0], "R": R[0], "S": S[0], "T": T[0], "U": U[0]}
    if not bool(
...
def ray_task(task: tuple[int, Fraction, Fraction]) -> dict:
    ...
    return {
        "kind": "order8_nested_curvature_dimensionless_finite_ray_block",
        "index": index,
        "mode_left": str(left),
        "mode_right": str(right),
        **result,
...
def compact_tail_records() -> list[dict]:
    ...
    collar = order8_compact.build_right_collar(order8_compact.DEFAULT_RIGHT_COLLAR)
    base, middle, top, ultra = order8_compact.append_right_collar(
        base,
        middle,
        top,
        ultra,
    ...
    rows = []
    for low_row, middle_row, top_row, ultra_row in zip(
        base,
        middle,
        top,
        ultra,
        ...
        merged["h_derivatives"] = {
            ...
            **ultra_row["h_derivatives"],
        }
        rows.append(merged)
    if not rows or rows[0]["mode_left"] != str(BASE_TAIL_START):
        raise RuntimeError("order-eight compact tail starts at the wrong mode")
    return rows


def merged_extension(ultra: list[dict]) -> list[dict]:
    tasks = extension_tasks()
    low = order6_ray.load_cache(order6_ray.DEFAULT_EXTENSION_CACHE, tasks)
    top = order7_ray.load_cache(order7_ray.DEFAULT_EXTENSION_CACHE, tasks)
    if len(low) != len(tasks) or len(top) != len(tasks) or len(ultra) != len(tasks):
        raise RuntimeError("u=2 extension caches are incomplete")
    rows = []
    for low_row, top_row, ultra_row in zip(low, top, ultra):
        endpoints = {
            (row["mode_left"], row["mode_right"])
            for row in (low_row, top_row, ultra_row)
        }
        if len(endpoints) != 1:
...
def initial_collar(extension: list[dict]) -> dict:
    ...
    if not bool(
        ...
        raise RuntimeError("u=2 extension cache does not contain a t+-6 collar")
    result = evaluate_dimensionless(
        MODE_TWO,
        COLLAR_END,
        derivatives,
    ...
    if not result.get("passed"):
        raise RuntimeError(f"initial order-eight u=2 collar failed: {result}")
    return result


def source_contract() -> dict:
    compact = load_json(order8_compact.DEFAULT_OUT)
    high = load_json(HIGH_CUMULANT_SOURCE)
    if compact.get("compact", {}).get("all_blocks_passed") is not True:
        raise RuntimeError("order-eight compact handoff changed")
    if high.get("exact", {}).get("finite_exact_corridor_cap") != ULTRA_CUMULANT_CAP:
        raise RuntimeError("order-eight high-cumulant source changed")
    inherited = order7_ray.source_contract()
    return {
        "exact_corridors": inherited["exact_corridors"],
        "mid_cumulants": inherited["mid_cumulants"],
        "top_cumulants": inherited["high_cumulants"],
        "ultra_cumulants": high["exact"]["finite_exact_corridor"],
        "compact_handoff": compact["compact"]["theorem"],
        "order7_extension_sha256": sha256(order7_ray.DEFAULT_EXTENSION_CACHE),
        "order8_high_cumulant_sha256": sha256(HIGH_CUMULANT_SOURCE),
        "order8_compact_sha256": sha256(order8_compact.DEFAULT_OUT),
    }
...
def finite_certificate(extension: list[dict], ray: list[dict]) -> dict:
    ...
    weakest_u = min(
        range(len(all_rows)),
        key=lambda index: Decimal(all_rows[index]["U_lower"]),
    )
    ...
    return {
        ...
        "weakest_U_index": weakest_u,
        "weakest_U_lower": all_rows[weakest_u]["U_lower"],
        "selected": [all_rows[index] for index in selected_indices],
        "theorem": "s_1''(t)<=4000/t^2 for every saddle mode 2<=u<=20",
    }
...
def build_artifact(extension: list[dict], ray: list[dict]) -> dict:
    ...
    rows = [
        RayRow(
            "co8ncfr_01_exact_derivatives",
            "exact_theorem_composition",
            "ready_to_apply",
            "Exact and coarse cumulant corridors through order fourteen enclose every required H derivative on the finite ray.",
            "H^(r) enclosed for r=2,...,14 on a t+-6 collar",
            "First Newman summand at lambda=-100 only.",
        ),
        RayRow(
            "co8ncfr_02_initial_collar",
            "interval_certificate",
            "ready_to_apply",
            "The aligned compact and extension quadrature caches bridge mode u=2 without using a corridor below its domain.",
            "s_1''(t)<=4000/t^2 for 2<=u<=2001/1000",
            "Finite aligned H2-H14 cache only.",
            finite["initial_collar"],
        ),
        RayRow(
            "co8ncfr_03_dimensionless_cover",
            "interval_theorem",
            "ready_to_apply",
            "A rational 1/1000-mode grid and dimensionless five-layer stable arithmetic prove the finite-ray curvature theorem.",
            finite["theorem"],
            "Outward-rounded Arb cover; no point sampling.",
            {
                ...
                "weakest_U_lower": finite["weakest_U_lower"],
            },
        ),
        RayRow(
            "co8ncfr_04_asymptotic_handoff",
            "open_handoff",
            "not_ready_to_apply",
            "Continue the dimensionless theorem on the asymptotic mode ray.",
            "s_1''(t)<=4000/t^2 for u>=20",
            "Open asymptotic ray only.",
        ),
    ]
    return {
        "kind": "jensen_window_pf_compound_order8_nested_curvature_finite_ray_certificate",
        "date": "2026-07-13",
        "status": "rigorous order-eight nested first-summand curvature theorem on 2<=u<=20",
        "proof_boundary": (
            "This artifact proves continuous first-summand curvature only on "
            "2<=u<=20. It does not prove the u>=20 ray, order-eight entry, "
        ...
        "generator": "work/rh_compute/scripts/jensen_window_pf_compound_order8_nested_curvature_finite_ray_certificate.py",
        "checker": "work/rh_compute/scripts/check_jensen_window_pf_compound_order8_nested_curvature_finite_ray_certificate.py",
    }
...
def write_note(path: Path, artifact: dict) -> None:
    ...
    lines = [
        ...
        "Date: 2026-07-13",
        "",
        "Status: rigorous first-summand order-eight curvature theorem on",
        "`2<=u<=20`. This is not a proof of the asymptotic ray, order-eight",
        "entry, PF-infinity, RH, or `Lambda <= 0`.",
        "",
        "## Dimensionless Cover",
        "",
        "Exact signed cumulant corridors through order eight, inherited coarse",
        "corridors for orders nine through twelve, and the unit corridors for",
        "orders thirteen and fourteen enclose `H^(2),...,H^(14)` on a strict",
        "`t+-6` collar. A short aligned quadrature extension handles `u=2`;",
        "the remaining interval uses rational mode width `1/1000`.",
        "",
        "```text",
        finite["theorem"],
        f"ray blocks={finite['ray_blocks']},",
        f"largest t^2*s_1'' upper={finite['largest_scaled_curvature_upper']},",
        f"weakest U_1 lower={finite['weakest_U_lower']}.",
        "```",
        "",
        "## Remaining Ray",
        "",
        "```text",
        "Prove s_1''(t)<=4000/t^2 for u>=20.",
        "```",
        "",
        "```text",
...
def main() -> int:
    ...
    print(
        f"order-eight finite-ray caches: extension={len(extension)}, "
    ...
    print(
        "wrote order-eight nested finite-ray certificate: "
