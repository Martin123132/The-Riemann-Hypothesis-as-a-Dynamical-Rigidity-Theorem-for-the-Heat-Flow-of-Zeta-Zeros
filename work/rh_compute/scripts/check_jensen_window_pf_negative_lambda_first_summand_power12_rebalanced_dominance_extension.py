#!/usr/bin/env python3
"""Validate the inverse-twelfth-power first-summand dominance extension."""
...
from jensen_window_pf_negative_lambda_first_summand_power12_rebalanced_dominance_extension import (  # noqa: E402
    DEFAULT_NOTE,
    DEFAULT_OUT,
    build_artifact,
)
...
def validate_artifact(path: Path) -> tuple[dict, list[DominanceIssue]]:
    ...
    expected_kind = (
        ...
        "power12_rebalanced_dominance_extension"
    )
    if artifact.get("kind") != expected_kind:
        issues.append(issue("artifact", "bad-kind", artifact.get("kind")))
    if "inverse-twelfth-power" not in str(artifact.get("status", "")):
    ...
    required = {
        "high_region_bound": "epsilon(a(k))<=17*exp(-3*pi*k^(2/5))<k^(-12), k>=320",
        "low_region_bound": "epsilon(0)*P_k(u<a(k))<k^(-12), k>=320",
        "full_tail_relative_bound": (
            "0<=delta_k=(M_k-M_k^(1))/M_k^(1)<2/k^12 "
            "for every integer k>=320"
        ),
        "moment_log_error": "0<=log(1+delta_k)<2/k^12, k>=320",
        "adjacent_B_error": (
...
def validate_note(path: Path) -> list[DominanceIssue]:
    ...
    required = (
        "Status: rigorous inverse-twelfth-power first-summand dominance",
        "This is not a proof of order ten",
        "a(k)=log(k)/10",
        "all fourteen Arb endpoint and derivative gates are strict",
        "<k^(-12), k>=320",
        "<2/k^12 for every integer k>=320",
    )
    issues = []
...
def main() -> int:
    ...
    if issues:
        for finding in issues:
            print(
                f"POWER12-DOMINANCE {finding.section} "
                f"[{finding.code}] {finding.detail}"
            )
        print(f"power-twelve first-summand dominance: {len(issues)} issues")
        return 1
    summary = artifact["summary"]
    print(
        "validated power-twelve rebalanced first-summand dominance: "
