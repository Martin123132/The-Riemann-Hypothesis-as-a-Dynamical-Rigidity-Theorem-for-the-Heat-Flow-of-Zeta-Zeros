#!/usr/bin/env python3
"""Validate the exact-cumulant formal-tail certificate."""
...
from jensen_window_pf_compound_order4_exact_cumulant_formal_tail_certificate import (  # noqa: E402
    DEFAULT_NOTE,
    DEFAULT_OUT,
    REPO_ROOT,
    build_artifact,
...
EXPECTED_SUMMARY = (
    "validated order-four exact cumulant formal tails: 8 rows, 0 issues, "
    "7 exact rows, 31 polynomial orders, 2 closed formal tails, "
    "3 open exact components"
)
...
def validate(artifact_path: Path, note_path: Path) -> list[Finding]:
    ...
    expected_summary = {
        ...
        "maximum_y_degree": 30,
        "tail_polynomial_orders": 31,
        "formal_tails_closed": 2,
        "open_exact_components": 3,
    }
    if artifact.get("summary") != expected_summary:
        findings.append(finding("summary", "mismatch", artifact.get("summary")))
    if artifact.get("weight_majorant", {}).get("total_coefficient_cap") != "4/3":
        findings.append(finding("weight", "bad-coefficient-cap", artifact.get("weight_majorant")))
    if artifact.get("tail_budgets", {}).get("common_one_sided_bound") != (
        "formal tail <=2^32*q^(-35/4) for |z|<=1"
    ):
        findings.append(finding("tails", "bad-common-bound", artifact.get("tail_budgets")))
    ...
    required = (
        ...
        "<4/3",
        "Y(q)=1+sqrt(32*log(q))",
        "formal tail <=2^32*q^(-35/4)",
        "each formal tail <1/(500000*q^3)",
        "each formal tail <1/(100000*u*q^3)",
        "Three exact-density components remain",
        "open targets",
...
def main() -> int:
    ...
    if findings:
        ...
        print(f"order-four exact cumulant formal tails: {len(findings)} issues")
        return 1
