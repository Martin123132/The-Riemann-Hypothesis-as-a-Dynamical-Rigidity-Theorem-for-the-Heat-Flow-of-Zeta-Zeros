#!/usr/bin/env python3
"""Validate the rigorous lambda=-100 signed order-eight prefix."""
...
def validate_artifact(path: Path) -> tuple[dict, list[PrefixIssue]]:
    ...
    if artifact.get("kind") != "jensen_window_pf_compound_order8_m100_prefix_certificate":
        issues.append(issue("artifact", "bad-kind", artifact.get("kind")))
    if "prefix through n=1242" not in str(artifact.get("status", "")):
    ...
    expected_summary = {
        ...
        "coefficients": 1257,
        "positive_Q7_rows": 1245,
        "positive_relative_Q7_rows": 1243,
    ...
    if len(rows) != 1243 or [row.get("n") for row in rows] != list(range(1243)):
        issues.append(issue("finite", "bad-row-cover", len(rows)))
    if any(row.get("Q8_sign") != "positive_by_signed_condensation" for row in rows):
        issues.append(issue("finite", "bad-q8-sign", rows))
    ...
    try:
        ...
        if not minimum_lower.is_finite() or minimum_lower <= Decimal(1) / Decimal(300):
            issues.append(issue("finite", "minimum-not-above-one-over-300", minimum_lower))
...
def validate_note(path: Path) -> list[PrefixIssue]:
    ...
    required = (
        ...
        "retained-integral extensions totaling 930 rows",
...
def main() -> int:
    ...
    if issues:
        for finding in issues:
            print(
                f"ORDER8-M100-PREFIX {finding.section} "
                f"[{finding.code}] {finding.detail}"
            )
        print(f"order-eight lambda=-100 prefix certificate: {len(issues)} issues")
        return 1
    finite = artifact["finite"]
    summary = artifact["summary"]
    print(
        "validated order-eight lambda=-100 prefix: "
        f"{summary['coefficients']} coefficients, "
        f"{summary['positive_Q7_rows']} positive Q7 values, "
        f"{summary['positive_relative_Q7_rows']} positive relative Q7 margins, "
        ...
        f"weakest n={finite['minimum_relative_n']} above 1/300"
    )
    return 0
