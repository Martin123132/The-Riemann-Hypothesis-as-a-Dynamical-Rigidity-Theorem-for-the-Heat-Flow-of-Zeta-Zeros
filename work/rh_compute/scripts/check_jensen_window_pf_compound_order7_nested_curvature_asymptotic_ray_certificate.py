#!/usr/bin/env python3
"""Validate the order-seven nested-curvature asymptotic certificate."""
...
from jensen_window_pf_compound_order7_nested_curvature_asymptotic_ray_certificate import (  # noqa: E402
    B0_FLOOR,
    B1_MAGNITUDE_FLOOR,
    COLLAR_RADIUS,
    DEFAULT_NOTE,
...
def validate_artifact(path: Path) -> tuple[dict, list[Issue]]:
    ...
    for order in ("11", "12"):
        if high_caps.get(order, {}).get("target_cap") != HIGH_B_CAP:
            issues.append(issue("ratios", f"cap-{order}", high_caps.get(order)))
    ...
    for key, count in (
        ("normalized_B_boxes", 11),
        ("normalized_J_boxes", 9),
        ("normalized_R_boxes", 7),
        ("normalized_S_boxes", 5),
        ("normalized_T_boxes", 3),
    ):
...
def main() -> int:
    ...
    for item in issues:
        print(f"ORDER7-NESTED-ASYMPTOTIC {item.section} [{item.code}] {item.detail}")
    if issues:
        print(f"order-seven nested asymptotic certificate: {len(issues)} issues")
        return 1
    summary = artifact["summary"]
    interval = artifact["dimensionless_interval"]
    print(
        "validated order-seven nested curvature asymptotic certificate: "
