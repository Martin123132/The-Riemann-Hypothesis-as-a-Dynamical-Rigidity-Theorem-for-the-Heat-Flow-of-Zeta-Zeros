#!/usr/bin/env python3
"""Reduce the lambda=-100 order-eight tail to one nested curvature ceiling."""
...
TAIL_FIRST_N = 1243
TAIL_FIRST_K = TAIL_FIRST_N + 7
CURVATURE_CONSTANT = 4300
...
def validate_sources() -> dict:
    ...
    reduction = load_json(ORDER8_REDUCTION)
    order4 = load_json(ORDER4_ENTRY_SOURCE)
    if prefix.get("summary", {}).get("positive_Q8_rows") != TAIL_FIRST_N:
        raise RuntimeError("order-eight prefix source does not end at n=1242")
    if prefix.get("summary", {}).get("open_analytic_tails") != 1:
        raise RuntimeError("order-eight prefix tail contract changed")
...
def exact_reduction() -> dict:
    center, r, p, u = sp.symbols("A r p U", positive=True)
    q6_center = center**6 * sp.exp(r)
    h5_center = center**5 * sp.exp(p)
    q7 = sp.factor(q6_center**2 * (1 - sp.exp(-u)) / h5_center)
    target = center**7 * sp.exp(2 * r - p) * (1 - sp.exp(-u))
    if sp.simplify(q7 - target) != 0:
        raise RuntimeError("canonical Q7 factorization failed")

    m, k = sp.symbols("m k", integer=True, nonnegative=True)
    comparison = sp.expand(
        sp.Integer(1757) * k**2
        - sp.Integer(250) * CURVATURE_CONSTANT * (2 * k + 1)
    )
    shifted = sp.expand(comparison.subs(k, TAIL_FIRST_K + m))
    polynomial = sp.Poly(shifted, m)
    coefficients = polynomial.all_coeffs()
    if any(value <= 0 for value in coefficients):
        raise RuntimeError("order-eight tail comparison is not coefficient-positive")
    ...
    return {
        ...
        "fifth_gap": "U(t)=6*B(t)-r(t-1)+2*r(t)-r(t+1)",
        "order8_coordinate": (
            "s(t)=2*r(t)-p(t)+log(1-exp(-U(t)))"
        ),
        "canonical_factorization": "Q_(7,n)=A_(n+6)^7*exp(s(n+6))",
        "canonical_factorization_residual": "0",
        "curvature_identity": (
            "E_n=log(Q_(7,n)*Q_(7,n+2)/Q_(7,n+1)^2)="
            "7*log(x_k)+W_k, W_k=s(k-1)-2*s(k)+s(k+1), k=n+7"
        ),
        "stable_margin": "M_n=exp(-E_n)-1",
        "sign_equivalence": "Q_(8,n)>0 iff M_n>0 iff E_n<0",
        "tail_index": "k=n+7, so n>=1243 iff k>=1250",
        "sufficient_ceiling": "W_k<=4300/k^2 for every real/integer k>=1250",
        "log_buffer": (
        ...
        "shifted_polynomial_k_1250_plus_m": str(shifted),
...
def build_artifact() -> dict:
    ...
    rows = [
        ...
        TailRow(
            "co8m100tcr_02_nested_coordinate",
            "exact_identity",
            "ready_to_apply",
            "One additional stable logarithm gives a cancellation-free coordinate for Q7.",
            exact["fifth_gap"] + "; " + exact["order8_coordinate"] + "; " + exact["canonical_factorization"],
            "Exact lower-cone factorization only.",
        ),
        TailRow(
            "co8m100tcr_03_curvature",
            "exact_reduction",
            "ready_to_apply",
            "Signed order eight is equivalent to one centered curvature lying below the seventh defect buffer.",
            exact["curvature_identity"] + "; " + exact["sign_equivalence"],
            "Exact logarithmic reduction only.",
        ),
        TailRow(
            "co8m100tcr_04_defect",
            "theorem_input",
            "ready_to_apply",
        ...
        TailRow(
            "co8m100tcr_06_conditional_tail",
            "conditional_theorem",
            "ready_to_apply",
            "The scalar curvature ceiling would close the complete order-eight endpoint tail.",
            "[W_k<=4300/k^2 for k>=1250] => [Q_(8,n)(-100)>0 for n>=1243]",
            "Conditional on the displayed scalar ceiling only.",
        ),
        TailRow(
            "co8m100tcr_07_open_curvature",
            "open_handoff",
            "not_ready_to_apply",
            "Prove the fifth-nested stable curvature ceiling on the analytic tail.",
            exact["sufficient_ceiling"],
            "This scalar analytic ceiling is not proved here.",
        ),
    ]
    return {
        "kind": "jensen_window_pf_compound_order8_m100_tail_curvature_reduction",
        "date": "2026-07-13",
        "status": (
            "exact order-eight endpoint-tail reduction with one open "
            "fifth-nested curvature ceiling"
        ),
        "proof_boundary": (
            "This artifact proves the canonical Q7 normalization, sign reduction, "
            "and tail arithmetic. It does not prove the 4300/k^2 curvature ceiling, "
        ...
        "generator": (
            ...
            "jensen_window_pf_compound_order8_m100_tail_curvature_reduction.py"
        ),
        "checker": (
            "work/rh_compute/scripts/"
            "check_jensen_window_pf_compound_order8_m100_tail_curvature_reduction.py"
        ),
    }
...
def write_note(path: Path, artifact: dict) -> None:
    ...
    lines = [
        ...
        "```text",
        exact["rational_comparison"],
        exact["shifted_polynomial_k_1250_plus_m"] + ">0 for m>=0,",
...
def main() -> int:
    ...
    print(
        "wrote order-eight tail curvature reduction: "
        f"{summary['rows']} rows, "
