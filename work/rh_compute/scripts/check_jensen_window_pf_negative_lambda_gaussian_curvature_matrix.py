#!/usr/bin/env python3
"""Validate the negative-lambda Gaussian curvature matrix."""
...
from jensen_window_pf_negative_lambda_gaussian_curvature_matrix import (  # noqa: E402
    DEFAULT_NOTE,
    DEFAULT_OUT_JSON,
    REPO_ROOT,
    build_diagnostics,
...
REQUIRED_NOTE_STRINGS = (
    ...
    "0 < B_k <= 2/(3*(2*k+1))",
    "positive Gaussian scale mixture",
    "pushes `x_k>=1`",
...
def validate_top_level(artifact: dict) -> list[dict]:
    issues: list[dict] = []
    if artifact.get("kind") != "jensen_window_pf_negative_lambda_gaussian_curvature_matrix":
    ...
    for key in (
        ...
        "source_heat_flow_cone_entry_target",
    ...
    for required in ("finite theorem-search", "does not prove", "all-k", "curvature", "lambda <= 0"):
...
def validate_recomputed(artifact: dict) -> list[dict]:
    ...
    expected = {
        ...
        "checked_x_max": 21,
        "gaussian_deficit_rows": 63,
        "gaussian_deficit_positive_rows": 63,
        "bounded_deficit_rows": 63,
        "bounded_deficit_positive_rows": 63,
        "raw_threshold_rows": 63,
        "raw_threshold_positive_rows": 63,
        "positive_threshold_rows": 63,
    }
...
def validate_rows_and_summary(artifact: dict) -> list[dict]:
    ...
    for required in ("exact_baseline", "exact_reduction", "finite_diagnostic", "rejected_template", "live_route"):
    ...
    for row in rows if isinstance(rows, list) else []:
        ...
        if not any(marker in boundary for marker in ("not", "only", "finite", "rejects", "live")):
            issues.append(issue(row_id, "weak-boundary", boundary))
        if row.get("role") == "rejected_template" and "reject" not in boundary:
            issues.append(issue(row_id, "missing-rejection-boundary", boundary))
    ...
    for required in ("gaussian", "deficit", "63 rows", "positive gaussian scale-mixture", "wrong way"):
        if required not in finding:
            issues.append(issue("summary", "missing-main-finding-text", required))
    invariants = " ".join(str(item) for item in artifact.get("invariants", [])).lower()
    for required in ("no row", "bounded curvature", "positive gaussian", "finite prefix", "lambda <= 0"):
...
def main() -> int:
    ...
    if args.json:
        ...
        for item in issues:
            print(f"JWPF-NEG-LAMBDA-GAUSSIAN-CURVATURE {item['section']} [{item['issue']}] {item['detail']}")
        print(
            "validated Jensen-window PF negative-lambda Gaussian curvature matrix: "
            f"{summary.get('matrix_rows')} matrix rows, "
            f"{summary.get('gaussian_deficit_rows')} positive-deficit rows, "
            f"{summary.get('bounded_deficit_rows')} bounded-deficit rows, "
            f"{summary.get('raw_threshold_rows')} raw-threshold rows, "
            f"{len(issues)} issues"
        )
