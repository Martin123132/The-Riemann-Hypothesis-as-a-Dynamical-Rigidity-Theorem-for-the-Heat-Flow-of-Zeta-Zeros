#!/usr/bin/env python3
"""Validate the second-next asymptotic-ray coefficient certificate."""
...
from jensen_window_pf_compound_order4_formal_cumulant_second_next_parity_asymptotic_ray_certificate import (  # noqa: E402
    DEFAULT_NOTE,
    DEFAULT_OUT,
    REPO_ROOT,
    build_artifact,
...
EXPECTED_SUMMARY = (
    "validated order-four second-next asymptotic ray: 8 rows, 0 issues, "
...
def validate(artifact_path: Path, note_path: Path) -> list[Finding]:
    ...
    required = (
        ...
        "0<L_r^(infinity)<2-1/(100u)",
        "|V^(r)-q*P_r(u)|<=100000*u^12",
        "|D_r-D_r^(infinity)|<=8000000000*eta(u)<1/(200u)",
        "holds for all `u>=2`",
        "No exact cumulant corridor",
...
def main() -> int:
    ...
    if findings:
        ...
        print(f"order-four second-next asymptotic ray: {len(findings)} issues")
        return 1
