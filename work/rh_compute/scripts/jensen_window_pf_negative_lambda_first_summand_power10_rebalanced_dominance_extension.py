#!/usr/bin/env python3
"""Prove inverse-tenth-power first-summand dominance with the rebalanced split."""
...
DEFAULT_NOTE = (
    REPO_ROOT
    / "outputs/"
    "jensen_window_pf_negative_lambda_first_summand_power10_rebalanced_dominance_extension.md"
)
POWER9_SOURCE = (
    REPO_ROOT
    / "work/rh_compute/results/"
...
def source_contract() -> dict:
    source = json.loads(POWER9_SOURCE.read_text(encoding="utf-8"))
    if source.get("summary", {}).get("full_tail_power") != 9:
    ...
    if source.get("summary", {}).get("positive_analytic_gates") != 14:
        raise RuntimeError("power-nine analytic gate count changed")
    bound = source.get("diagnostics", {}).get("full_tail_relative_bound")
    if bound != (
        "0<=delta_k=(M_k-M_k^(1))/M_k^(1)<2/k^9 "
        "for every integer k>=300"
    ):
        raise RuntimeError("power-nine dominance contract changed")
    return {
...
def build_artifact() -> dict:
    ...
    rows = [
        ...
        DominanceRow(
            "fsd10r_04_low_region",
            "interval_analytic_certificate",
            "ready_to_apply",
            "The tilted first-summand probability below the split has enough reserve for a tenth inverse power.",
        ...
        DominanceRow(
            ...
            "analytic_theorem",
            "ready_to_apply",
            "The two regions compose to an inverse-tenth-power complete-to-first moment defect.",
            diagnostics["full_tail_relative_bound"],
            "Lambda=-100 integer moments only.",
        ),
        DominanceRow(
            "fsd10r_06_wall_error",
            "exact_consequence",
            "ready_to_apply",
            "The adjacent logarithmic wall inherits the inverse-tenth-power stencil bound.",
            diagnostics["moment_log_error"] + "; " + diagnostics["adjacent_B_error"],
            "Uses log(1+z)<=z and the exact centered stencil.",
        ),
    ]
    return {
        "kind": "jensen_window_pf_negative_lambda_first_summand_power10_rebalanced_dominance_extension",
        "date": "2026-07-13",
        "status": "rebalanced all-k inverse-tenth-power first-summand dominance extension",
        "proof_boundary": (
            "This artifact strengthens first-summand dominance only for lambda=-100 "
            "and k>=300. It does not prove the order-nine curvature ceiling, "
        ...
        "generator": (
            ...
            "jensen_window_pf_negative_lambda_first_summand_power10_rebalanced_dominance_extension.py"
        ),
        "checker": (
            "work/rh_compute/scripts/"
            "check_jensen_window_pf_negative_lambda_first_summand_power10_rebalanced_dominance_extension.py"
        ),
    }
...
def main() -> int:
    ...
    print(
        "wrote power-ten rebalanced first-summand dominance: "
        f"{summary['rows']} rows, "
