#!/usr/bin/env python3
"""Validate signed order-eight forward invariance through lambda zero."""
...
from jensen_window_pf_compound_order8_uniform_heat_forward_invariance_certificate import (  # noqa: E402
    DEFAULT_NOTE,
    DEFAULT_OUT,
    ENTRY_SOURCE,
    FLOW_SOURCE,
...
def validate_artifact(path: Path) -> tuple[dict, list[ForwardIssue]]:
    ...
    if "a_n=(4*n+58)" not in str(sources.get("cooperative_flow", "")):
        issues.append(issue("sources", "bad-cooperative-flow", sources))
    if "there exists N_8" not in str(sources.get("uniform_tail", "")):
    ...
    if "epsilon_8*R_(8,n)" not in str(theorem.get("arbitrary_order_eight", "")):
        issues.append(issue("theorem", "bad-arbitrary-order-eight", theorem))
...
def validate_note(path: Path) -> list[ForwardIssue]:
    ...
    required = (
        "Status: signed contiguous and arbitrary-column order-eight theorem",
        "not PF-infinity",
        "Q_(8,n)(-100)=H_(8,n)(-100)>0 for every integer n>=0",
        "there exists N_8",
        "Q_n'=a_n*Q_(n+1)+b_n*Q_n",
        "Q_(8,n)(lambda)=H_(8,n)(lambda)>0 for every n>=0",
        "epsilon_8*R_(8,n)",
        "contiguous and arbitrary-column order eight are both closed",
...
def main() -> int:
    ...
    if issues:
        for finding in issues:
            print(
                f"ORDER8-UNIFORM-FORWARD {finding.section} "
                f"[{finding.code}] {finding.detail}"
            )
        print(f"order-eight uniform-heat forward certificate: {len(issues)} issues")
        return 1
    summary = artifact["summary"]
    print(
        "validated order-eight uniform-heat forward invariance: "
